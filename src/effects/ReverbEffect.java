...
public class ReverbEffect implements Effect {
	 ...
	 */
	private static final int NUM_EARLY_LINES = 6;
	 ...
	 */
	private short[] earlyEchoes = new short[NUM_EARLY_LINES];

	/**
	 * Stores the echoes generated by the comb filters.
	 */
	private short[] lateEchoes = new short[NUM_LATE_LINES];
	...
	public final short process(short audioIn) {
		...
		/* Create late echoes */
		for (int i = 0; i < NUM_LATE_LINES; i++) {
			lateEchoes[i] = lateDelayLines[i].getReponse(audioIn);
			audioIn += lateEchoes[i] * ATTENUATION;
		}
		lowPassFilter.process(audioIn);

		/* Create early echoes */
		for (int i = 0; i < NUM_EARLY_LINES; i++) {
			x = (short) (echo + earlyEchoes[i] * ATTENUATION);
			earlyEchoes[i] = earlyDelayLines[i].getReponse(x);
			echo = (short) (x * -ATTENUATION + earlyEchoes[i]);
		}
		/* Apply high frequecies cutting */
		return (short) (audioIn + echo);
	}
	...
	public final void update(final Observable o, final Object arg) {
		if (o.equals(gain)) {
			...
			for (int i = 0; i < NUM_EARLY_LINES; i++) {
				...
				int delayTime = (int) (roomSize.getValue() / (10 * Math
						.pow(3, i)));
				earlyDelayLines[i].getDelayLength().setValue(delayTime);
			}
			for (int i = 0; i < NUM_LATE_LINES; i++) {
				int delayTime = (int) (roomSize.getValue() / (6 * Math.pow(3, i)));
